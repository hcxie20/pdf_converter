...
pdf_dir = "/Users/xiehaochen/Desktop/hospital/第二医院"
...
def convert_to_image(file_name, file_path):
    ...
    for i in range(len(images)):
        print(os.path.join(file_path, file_name[:-4]) + '_page_' + ('{0}'.format(i + 1)).zfill(len(str(len(images)))) + '.jpg')
        images[i].save(os.path.join(file_path, file_name[:-4]) + '_page_' + ('{0}'.format(i + 1)).zfill(len(str(len(images)))) + '.jpg')
